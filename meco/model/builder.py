...
def build_model(model_path, config=None, device='cpu', low_cpu_mem_usage=True, dtype=torch.float16, **kwargs):
    ...
    if nncore.is_file(adapter_path) or nncore.is_file(partial_path):
        ...
        model = AutoModelForCausalLM.from_pretrained(
            ...
            config=config,
            attn_implementation='eager' if getattr(config, 'bi_attention', False) else "flash_attention_2",
        ...
        if model.config.pretrain_vision_tower:
            model.model.vision_tower.load_weights(model.config.pretrain_vision_tower)

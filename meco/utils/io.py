...
def _load_video(video_path, fps=1, min_len=-1, max_len=999, **kwargs):
    ...
    if len(frame_idx) > max_len:
        # uniformly sample frames
        frame_idx = [frame_idx[i] for i in np.linspace(0, len(frame_idx) - 1, max_len).round().astype('int')]

    video = vr.get_batch(frame_idx)
    # bug in original etchat code    
    # tag = [round(i * avg_fps, 1) for i in frame_idx]
    tag = torch.tensor([round(i / avg_fps, 1) for i in frame_idx])

...
class MeCoTrainer(Trainer):
    ...
    def _get_train_sampler(self):
        if self.args.group_by_modality:
            ...
            return super(MeCoTrainer, self)._get_train_sampler()

    def _load_from_checkpoint(self, resume_from_checkpoint, model=None):
        if model is None:
            model = self.model

        super(MeCoTrainer, self)._load_from_checkpoint(resume_from_checkpoint, model=model)
    ...
    def _save_checkpoint(self, model, trial, metrics=None):
        super(MeCoTrainer, self)._save_checkpoint(model, trial, metrics=metrics)

...
def get_target_modules(model, lora_type, language_model):
    ...
    if lora_type == 'linear':
        excludes = ['vision_tower', 'mm_projector', 'qformer', 'lm_head', 'frm_head', 'str_head']
        target_modules = set()
                ...
                if mm_type == 'image':
                    ...
                    if  self.model_args.localization:
                        ...
                        if "tst_windows" in anno:
                            ...
                            # windows: N x 2
                            # tag: T
                            # Upate windows with sampled timestamps (tag) if frame sampling is used (not used in reality)
...
def train():
    ...
    for n, p in model.named_parameters():
        if config.localization and any(k in n for k in ('embed_tokens', 'lm_head', 'frm_head', 'str_head', "logit_scale")):
